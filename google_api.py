...
# Date: 16-12-2024
# Version : 2.0
# Version Comments: Added utf-8 encoding to api url to handle special characters and replace single quote (') in address with double quotes (") in address
# Objective: Fetching data from S3 and hit Google Maps API fill values in data and ingest it to Redshift
# User Story:
########################################################
...
def fetch_data_api(address):
    ...
    try:
        ...
        url=config["url"].replace("'",'"') # replaces single quote (') in address with double quotes (")
        url = url.format(address=quote(address, encoding='utf-8')) # formats url with address and encodes it to utf-8 format
        from api_connector import ApiRequest # api_connector Module imported for hitting API
        api=ApiRequest(logger=logger)
        response=api.get_request(api_url=url)
        ...
        if "s3_profile" in config:
            ...
            for process_file in to_be_processed_files:
                ...
                for index,data in df.iterrows():
                    ...
                    if location:
                        df.at[index,"latitude"]=float(location["lat"]) # updates the particular column in the row with value generated from API
                        df.at[index,"longitude"]=float(location["lng"]) # updates the particular column in the row with value generated from API
